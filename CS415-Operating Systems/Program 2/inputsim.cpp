...
void simulateInput(int rate)
{
	...
	uniform_int_distribution<int> uD(1, 10); // Had to update this to reflect 10 values and not 11 (0-10 == 11)
...
void simulateOutput(string line)
{
	string delimiter = " ";
	size_t pos = 0;
	int incr = 0;
	...
	if (stry[2] != "START") {
		string fileName = stry[1] + ".txt";
		fstream file(fileName, ios::in | ios::out | ios::app);
		file << stry[0] << "\t" << stry[1] << "\t" << stry[2] << endl;
	}
}
